...
func SendText(c *gin.Context) {
	...
	var reqId int
	if data.AtIds != "" {
		text, reqId = _struct.SendText(_struct.Config.Robot[0].BotWxid, data.ReceiverId, data.Text, data.AtIds)
	} else {
		text, reqId = _struct.SendText(_struct.Config.Robot[0].BotWxid, data.ReceiverId, data.Text, "")
	}
...
func SendImage(c *gin.Context) {
	...
	image, reqId := _struct.UploadCdnImg(_struct.Config.Robot[0].BotWxid, data.ReceiverId, data.ImageUrl)
...
func SendAppMsg(c *gin.Context) {
	...
	appMsg, reqId := _struct.SendAppMessage(_struct.Config.Robot[0].BotWxid, data.ReceiverId, data.Xml, 49)
...
func SendPat(c *gin.Context) {
	...
	pat, reqId := _struct.SendPatMessage(_struct.Config.Robot[0].BotWxid, data.ReceiverId, data.PatId, 0)
...
func SendEmoji(c *gin.Context) {
	...
	emoji, reqId := _struct.SendEmoji(_struct.Config.Robot[0].BotWxid, data.ReceiverId, data.EmoJiMd5, data.EmoJiLength)
...
func SendVoice(c *gin.Context) {
	...
	voice, reqId := _struct.SendVoice(_struct.Config.Robot[0].BotWxid, data.ReceiverId, data.VoiceUrl, data.VoiceLength)
